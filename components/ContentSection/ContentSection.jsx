      ...
      .then((allData) => {
        if (allData.status === "success") {
          ...
          toast.current.show({
            severity: "error",
            summary: "Error",
            detail: "you are not logged in please login first",
            life: 3000,
          });
          console.log(allData);
        }
    ...
    if (data) {
      if (data) {
        const found = data.items.some(
          (item) => item.product.id === productId.split("/")[2],
        );
        setIsFilled(found);
      }
    }
  ...
  return (
    <div className={styles.contentSection}>
      ...
      <div className={styles.addToWishList}>
        <i
          className={`bi ${
            !isFilled ? "bi-heart" : "bi-heart-fill text-danger"
          }`}
